...
/**
 *  行情缩略图 给于标签调用 拿出商品表的 original_img 原始图来裁切出来的
 * @param type $article_id  商品id
 * @param type $width     生成缩略图的宽度
 * @param type $height    生成缩略图的高度
 */
function hqarticle_thum_images($article_id,$width,$height){

     if(empty($article_id))
         return '';
    //判断缩略图是否存在
    $path = "Public/upload/market/thumb/$article_id/";
    $article_thumb_name ="article_thumb_{$articel_id}_{$width}_{$height}";
  
    // 这个行情文章 已经生成过这个比例的图片就直接返回了
    if(file_exists($path.$article_thumb_name.'.jpg'))  return '/'.$path.$article_thumb_name.'.jpg'; 
    if(file_exists($path.$article_thumb_name.'.jpeg')) return '/'.$path.$article_thumb_name.'.jpeg'; 
    if(file_exists($path.$article_thumb_name.'.gif'))  return '/'.$path.$article_thumb_name.'.gif'; 
    if(file_exists($path.$article_thumb_name.'.png'))  return '/'.$path.$article_thumb_name.'.png'; 
        
    $original_img = M('hq_article')->where("article_id = $article_id")->getField('thumb');
    if(empty($original_img)) return '';
    ...
    //参考文章 http://www.mb5u.com/biancheng/php/php_84533.html  改动参考 http://www.thinkphp.cn/topic/13542.html
    $image->thumb($width, $height,3)->save($path.$article_thumb_name,NULL,100); //按照原图的比例生成一个最大为$width*$height的缩略图并保存
    
    //图片水印处理
    $water = tpCache('water');
    if($water['is_mark']==1){
        $imgresource = './'.$path.$article_thumb_name;
        if($width>$water['mark_width'] && $height>$water['mark_height']){
            if($water['mark_type'] == 'img'){
                $image->open($imgresource)->water(".".$water['mark_img'],$water['sel'],$water['mark_degree'])->save($imgresource);
            }else{
    ...
    return '/'.$path.$article_thumb_name;
}
...
/**
     ...
     * @return bool    短信发送成功返回true失败返回false
     */
    //function sendSMS($mobile, $smsSign, $smsParam, $templateCode)
	function sendSMS($mobile, $code)
    {
        include_once VENDOR_PATH . 'aliyun-php-sdk-core/Config.php';
        include_once VENDOR_PATH . 'Dysmsapi/Request/V20170525/SendSmsRequest.php';
        
		$config =  tpCache('sms');
		$accessKeyId = $config['sms_appkey'];//阿里云短信 keyId
		$accessKeySecret = $config['sms_secretKey'];//阿里云短信 keysecret
...
function update_pay_status($order_sn,$pay_status = 1)
{
    ...
    function confirm_order($id,$user_id = 0){
        ...
        if($row){
            $user_order=M('order_goods')->where(array('order_id'=>$id,'prom_type'=>4))->getField('rec_id,order_id,goods_id,goods_num,member_goods_price,act_id');
            if($user_order){
...
function get_goods_promotion($goods_id,$user_id=0,$act_id=0){
	$now = time();
    ...
    if($goods['prom_type']==4){//拼单
        $prominfo = M('application_buy')->where(array('id'=>$act_id))->find();
        if(!empty($prominfo)){          
            if($prominfo['goods_num'] == $prominfo['buy_num']){
                $prom['is_end'] = 2;//已售馨
            }else{
...
{    
    ...
    $goods_arr = M('goods')->where("goods_id in(".  implode(',',$goods_id_arr).")")->getField('goods_id,weight,market_price,is_free_shipping'); // 商品id 和重量对应的键值对
        ...
        {       
            ...
            if($goods_arr[$val['goods_id']]['is_free_shipping'] == 0)
	            $goods_weight += $goods_arr[$val['goods_id']]['weight'] * $val['goods_num']; //累积商品重量 每种商品的重量 * 数量
                        ...
                        //商品满减折扣计算
           $num = M()->query("select nummin,nummax from tp_discount where is_show=1");

             
            foreach ($num as $key => $value) {
                
                 if($val['goods_num']>=$value['nummin']&&$val['goods_num']<=$value['nummax']){
                    
                          $discount=M('discount')->where(array('nummin'=>$value['nummin'],'nummax'=>$value['nummax']))->getField('discount');
                          // dump(print_r($discount));
                          $val['member_goods_price'] = $val['member_goods_price'] * $discount/100;
                }
               
              }
            
            $nummax=M()->query("select max(nummax) from tp_discount");
            if($nummax[0]['max(nummax)']<$val['goods_num']){
             $discount=M('discount')->where(array('nummax'=>$nummax[0]['max(nummax)']))->getField('discount');
             $val['member_goods_price'] = $val['member_goods_price'] * $discount/100;                
            }
			...
			//如果有商品重量，则商品价格=重量*价格
            $order_goods[$key]['goods_fee'] = $val['goods_weight'] ? $val['goods_weight'] * $val['shop_price_jin'] : $val['goods_num']  * $val['member_goods_price'];    // 小计
        ...
        $first = M('users')->where('user_id = '.$user[user_id])->getField('total_amount');//首单95折
        if($first==0){
            $goods_price = $goods_price * 1.00;
        ...
        if($shipping_price == 0 )
        {
            ...
            if($freight_free > 0 && $goods_price >= 800)
               $shipping_price = 0 ;               
        }
