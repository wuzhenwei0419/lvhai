...
class AgentGoodController extends MobileBaseController  {
    ...
    */ 
    public function index(){
        //form表单提交
         ...
         $goodsLogic = new \Mobile\Logic\GoodsLogic();
         $id = I("get.id");
         $user = M('application_buy')->where("id = $id ")->getField('id,goods_id,user_phone');
         //var_dump(print_r($user));
          $goods_id = $user[$id]['goods_id'];
          $phone_num = $user[$id]['phone_num'];
          //var_dump(print_r($goods_id));
          $group_buy_info = M('application_buy')->where("id = $id and ".time()." >= start_time and ".time()." <= end_time ")->find(); // 找出这个商品
        //  //var_dump(print_r($group_buy_info));
         if(empty($group_buy_info)) 
         {
             $this->error("此商品没有拼单活动",U('Home/Goods/goodsInfo',array('id'=>$goods_id)));
             exit;
         }
