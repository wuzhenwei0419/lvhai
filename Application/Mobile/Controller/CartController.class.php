...
class CartController extends MobileBaseController {
    ...
    function addCart()
    {
        ...
        $act_id = I("id");
        $result = $this->cartLogic->addCart($goods_id, $goods_num, $goods_spec,$act_id,$unique_id,$user_id); // 将商品加入购物车
    ...
    function ajaxAddCart()
    {
        ...
        $goods_spec = I("goods_spec"); // 商品规格
        $act_id = I("id");
        $result = $this->cartLogic->addCart($goods_id, $goods_num, $goods_spec,$act_id,$this->session_id,$this->user_id); // 将商品加入购物车
        exit(json_encode($result));
    }
    ...
    */
    public function ajaxCartList()
    {
        ...
        $prom_type = M('Cart')->where($where)->getField("prom_type");
        ...
        if($post_goods_num)
        {
            ...
            {
                $quantity[$key]=M('goods')->where(array('goods_id'=>$cartList[$key]['goods_id']))->getField('quantity');
                $data['goods_num'] = $val < $quantity[$key] ? $quantity[$key] : $val;
                //var_dump(print_r($quantity[$key]));
            ...
            $this->assign('select_all', $_POST['select_all']);
            // 全选框
        }
