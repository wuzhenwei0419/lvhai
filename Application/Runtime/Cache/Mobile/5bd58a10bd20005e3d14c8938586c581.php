...
<div class="goods_nav hid" id="menu">
      ...
      <ul>
        ...
        <li><a class="menua3" href="<?php echo U('Cart/cart');?>"><span class="menu3"></span><i>购物车</i></a></li>
        <li style=" border:0;"><a href="<?php echo U('User/index');?>"><span class="menu4"></span><i>我的</i></a></li>
   </ul>
 </div>
<script>
    var menua3=document.querySelector(".menua3")
    console.log(menua3)
    var id=getCookie("id")
    if(id==""){
        menua3.setAttribute("href","<?php echo U('User/login');?>")
    }else {
        menua3.setAttribute("href","<?php echo U('Cart/cart');?>")
    }
</script>
    <div class="main" id="user_goods_ka_1" style="display:block;">
        ...
        <form name="buy_goods_form" method="post" id="buy_goods_form" >
            <div class="product_info">
                <div class="info_dottm" style="float: left;" >
                    <h3 class="name" style="float:left;padding-left: 10%"><?php echo ($goods["goods_name"]); ?></h3>
                </div>
                <div class="info_dottm" onclick="Birth($(this))" style="float: right;padding-right: 2.5%">
                    <img width="auto" height="auto" src="/Template/mobile/new/Static/images/fen.png" alt=""  >
                </div>
                <!-------商品属性-------->
                <?php if($filter_spec != ''): ?><section id="search_ka">
                        <!---属性---->
                        <div class="ui-sx bian1">
                            <div class="subNavBox goods-spec">
                                <ul class="navContent">
                                    <?php if(is_array($filter_spec)): foreach($filter_spec as $key=>$spec): ?><li>
                                            <div class="title">选择<?php echo ($key); ?></div>
                                            <div class="item">
                ...
                <dl class="goods_price">
                    ...
                    <dt> <span id="goods_price" class="islogin">￥<?php echo ($goods["shop_price"]); ?>元</span><a href="<?php echo U('User/index');?>" class="nologin">登录查看最新价格</a><font>价格：￥<?php echo ($goods["market_price"]); ?>元</font> </dt>
                    <dd style="font-size: 12px; line-height: 1; margin-bottom: 5px;" class="disappear">
                        <i class="onlylost">仅剩
                            <em>
                                <strong id="countdown" style="color:#db384c">
                                    <script type="text/javascript">
                                        function GetRTime2(){
                                            $("#countdown").text(GetRTime('<?php echo (date("Y/m/d H:i:s",$goods['flash_sale']['end_time'])); ?>'));
                                        }
                                        setInterval(GetRTime2,999);
                                        if (document.referrer === 'http://localhost/index.php/Mobile/Index/promoteList.html') {
            ...
            <?php if(($prom_goods != null) OR ($prom_order != null)): ?><section id="search_ka" class="huodong">
                    ...
                    <div class="navContent">
                        <?php if($prom_goods != null): ?><ul class="youhui_list1">
                                <li>
                                    <a href="javascript:void(0);" title="<?php echo ($prom_goods[name]); ?>"><img width="auto" height="auto" src="/Template/mobile/new/Static/images/hui.png"></a>
 ...
 $md5_key = md5("select * from `__PREFIX__prom_order` order by  id desc limit 5"); $result_name = $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__prom_order` order by  id desc limit 5"); S("sql_".$md5_key,$sql_result_v,31104000); } foreach($sql_result_v as $k=>$v): ?><ul class="youhui_list1" style="margin-top:0px;">
                                <li><img width="auto" height="auto" src="/Template/mobile/new/Static/images/hui.png"><?php echo ($v['name']); ?>&nbsp;&nbsp;(活动时间：<?php echo (date("m月d日",$v[start_time])); ?> - <?php echo (date("m月d日",$v[end_time])); ?>)</li>
                            </ul><?php endforeach; ?>
            ...
            <section id="search_ka">
                <div class="ui-sx bian1">
                    <div class="subNavBox">
                        <div class="subNav on"><strong>优惠活动</strong></div>
 ...
 foreach ($discount as $key => $value) { echo $value['nummin']; echo '-'; echo $value['nummax']; echo '件'; echo $value['discount']/10; echo '折；  '; } ?></p>
            ...
            <section id="search_ka">
                <div class="ui-sx bian1">
                    <div class="subNavBox">
                        <div class="subNav on"><strong>温馨提示</strong></div>
                        <ul class="navContent" style="display: block;">
                            <li style=" border-bottom:1px solid #eeeeee">
                                <p class="tips">当天下单，次日配送 。<br>拼单和一件代发商品，可以快递全国，只要快递能到达的。</p >
                            </li>
                        </ul>
                        </ul>
                    </div>
                ...
                <div class="ui-sx bian1">
                    <div class="subNavBox">
                        ...
                        <ul class="navContent" style="display: block;">
                            <li style=" border-bottom:1px solid #eeeeee">
                                <div class="item1">
                  <span class="ui-number" style="display: inline-block; vertical-align: middle;">
                  <button type="button" class="decrease" onClick="goods_cut();">-</button>
                    <input type="text" class="num" id="number" name="goods_num" value="<?php echo ($goods["quantity"]); ?>" min="1" max="1000" onkeyup="value=value.replace(/[^\d]/g,'')"/>
                    <input type="hidden" name="goods_id" value="<?php echo ($goods["goods_id"]); ?>"/>
                  <button type="button" class="increase" onClick="goods_add();">+</button>
                  </span>
                                    <span style="margin-left: 10px;margin-top: 10px;display: inline-block;vertical-align: middle; font-size: 14px; color: #333;">库存：<?php echo ($goods["store_count"]); ?>&nbsp &nbsp起订数量:</span><span id="numMin" style="margin-left: 10px;margin-top: 10px;display: inline-block;vertical-align: middle; font-size: 14px; color: #333;"><?php echo ($goods["quantity"]); ?></span>
                                </div>
                            </li>
                        </ul>
                    </div>
            ...
            <p class="goods-remark"><?php echo ($goods["goods_remark"]); ?></p>
            <!--<section id="search_ka">-->
            <!--<div class="ui-sx bian1">-->
            <!--<div class="subNavBox" >-->
...
function changeAtt1(t) {
	...
	function goods_cut(){
  		...
  		if(Num><?php echo ($goods["quantity"]); ?>)Num=Num-1;
  		num_val.value=Num;
  	}
  	function goods_add(){
  		var num_val=document.getElementById('number');
...
<div class="footer_nav">
    ...
    <dl>
        <dd class="flow"><a class="button active_button" href="javascript:void(0);" onClick="AjaxAddCart(<?php echo ($goods["goods_id"]); ?>,1,0,<?php echo ($goods["is_collage"]); ?>);">加入购物车</a> </dd>
        <dd class="goumai">
            <a id="a" style="display:block;" href="javascript:void(0);"  onclick="AjaxAddCart(<?php echo ($goods["goods_id"]); ?>,1,0,<?php echo ($goods["is_collage"]); ?>);">立即购买</a>
        </dd>
    </dl>
</div>
<div class="htt" style="float: right;display: none" >  http://www.lvhai.shop/index.php/Mobile/Goods/goodsInfo/id/<?php echo ($goods["goods_id"]); ?></div>
<script type="text/javascript">
    //购物车
    //购物车
    function con() {
    ...
    var show_cart=document.querySelector(".show_cart")
    if(id==""){
        a.onclick=function () {
            con()
        }
        a.setAttribute("href","/index.php/Mobile/User/index.html")
        show_cart.setAttribute("href","/index.php/Mobile/User/index.html")
    }else {
        a.onclick=function () {
            AjaxAddCart(<?php echo ($goods["goods_id"]); ?>,1,0,<?php echo ($goods["is_collage"]); ?>)
        }
        a.setAttribute("href","/index.php/Mobile/Cart/cart.html")
        show_cart.setAttribute("href","/index.php/Mobile/Cart/cart.html")
    }
    $(document).ready(function(){
        // 更新商品价格
        get_goods_price();
...
function Birth($ele) {
    ...
    layer.open({
        ...
        '\t\t\t\t<br />\n' +
        '\t\t\t\t<br />\n' +
        '\t\t\t\t<div id="qrcode" style="margin-left:30px;margin-right:30px">\n' +
        '\t\t\t\t</div>\n' +
        '\t\t\t\t<br />\n'
    });
