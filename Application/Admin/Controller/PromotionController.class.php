...
class PromotionController extends BaseController {
    ...
    public function application_buy_list(){
        $Ad =  M('application_buy');
        $count = $Ad->count();
        $Page = new \Think\Page($count,10);  
        // var_dump(print_r(session('admin_id')));
        $admin_id=session('admin_id');
        $role_id = M('admin')->where(array('admin_id'=>$admin_id))->getField('role_id');
        if($role_id == 18){
            $phone = M('admin')->where(array('admin_id'=>$admin_id))->getField('phone_num');
            $res = $Ad->where(array('user_phone'=>$phone))->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        }else{
        $res = $Ad->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        }
        if($res){
    ...
    public function application_buy(){
        $act = I('GET.act','add');
        $groupbuy_id = I('get.id');
        $group_info = array();
        $group_info['start_time'] = date('Y-m-d');
        $group_info['end_time'] = date('Y-m-d',time()+3600*365);
        if($groupbuy_id){
            $group_info = D('application_buy')->where('id='.$groupbuy_id)->find();
            $group_info['start_time'] = date('Y-m-d H:i',$group_info['start_time']);
            $group_info['end_time'] = date('Y-m-d H:i',$group_info['end_time']);
            $act = 'edit';
        }
    ...
    public function applicationHandle(){
        $data = I('post.');
        $data['groupbuy_intro'] = htmlspecialchars(stripslashes($_POST['groupbuy_intro']));
        $data['start_time'] = strtotime($data['start_time']);
        $data['end_time'] = strtotime($data['end_time']);
        ...
        if($data['act'] == 'del'){
            $s=D('application_buy')->where('id='.$data['id'])->getField('goods_id');
            $r = D('application_buy')->where('id='.$data['id'])->delete();
            if($s=='')
            M('goods')->where("prom_type= 4 and prom_id=".$data['id'])->save(array('prom_id'=>0,'prom_type'=>0));
            if($r) exit(json_encode(1));
        }
        if($data['act'] == 'add'){
            $r = D('application_buy')->add($data);
            M('goods')->where("goods_id=".$data['goods_id'])->save(array('prom_id'=>1,'prom_type'=>4));
        }
        if($data['act'] == 'edit'){
            $r = D('application_buy')->where('id='.$data['id'])->save($data);
             M('goods')->where("prom_type=4 and prom_id=".$data['id'])->save(array('prom_id'=>0,'prom_type'=>0));
             M('goods')->where("goods_id=".$data['goods_id'])->save(array('prom_id'=>1,'prom_type'=>4));
        }
        if($r){
            $this->success("操作成功",U('Admin/Promotion/application_buy_list'));
        }else{
            $this->error("操作失败",U('Admin/Promotion/application_buy_list'));
        }
    }


    public function search_application_goods(){
        $GoodsLogic = new \Admin\Logic\GoodsLogic;
        $brandList = $GoodsLogic->getSortBrands();
        $this->assign('brandList',$brandList);
        $categoryList = $GoodsLogic->getSortCategory();
        ...
        $goods_id = I('goods_id');
        $where = ' is_on_sale = 1  and store_count>0 and is_collage= 1';//搜索条件
        if(!empty($goods_id)){
            $where .= " and goods_id not in ($goods_id) ";
        }
        I('intro')  && $where = "$where and ".I('intro')." = 1";
        ...
        $this->assign('page',$show);//赋值分页输出
        $this->assign('goodsList',$goodsList);
        $tpl = I('get.tpl','search_application_goods');
        $this->display($tpl);
    }
    ...
    public function save_password(){
        $password=I('post.order_password');
        $password1=I('post.password1');
        $password2=I('post.password2');
        $order_password=encrypt($password);
        $admin_id=session('admin_id');
        $data=M('admin')->where(array('admin_id'=>$admin_id))->getField('password');
        if($order_password==$data){
            if($password1==$password2){
                $save['password']=encrypt($password2);
                $r=M('admin')->where(array('admin_id'=>$admin_id))->save($save);
            }
        }
        if($r){
            $this->success('修改成功',U('Admin/promotion/application_buy_list'));
        }else{
            $this->error('修改失败',U('Admin/promotion/change_password'));
        }
