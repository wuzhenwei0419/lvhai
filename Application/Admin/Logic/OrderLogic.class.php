...
class OrderLogic extends RelationModel
{
     ...
     */
    public function getOrderList($condition,$order='',$start=0,$page_size=20,$order_triage=0){ 
        if($order_triage==859){
           $goods_id=array();
           $goods_id= M('goods')->where(array('cat_id'=>859))->select();
           foreach ($goods_id as $key => $value) {
              $order_id= M('order_goods')->where(array('goods_id'=>$value['goods_id']))->select();
           ...
           $sql="select * from tp_order where order_id  in (".$data."-1)";
           if($condition['order_sn']!=''){
            $sql.=" and order_sn =".$condition['order_sn'];
           }
           if($condition['consignee']!=''){
            $sql.=" and consignee = '".$condition['consignee']."'";
           }
           if($condition['pay_status']!=''){
            $sql.=" and pay_status =".$condition['pay_status'];
           }
           if($condition['pay_code']!=''){
            $sql.=" and pay_code = '".$condition['pay_code']."'";
           }
           if($condition['shipping_status']!=''){
            $sql.=" and shipping_status =".$condition['shipping_status'];
           }
           ...
           if($condition['add_time']!=''){
            ...
            $sql.=" and add_time ".$s.$l;
           }
      
         $res=M()->query($sql);
            
     }
     elseif($order_triage==1){
        $goods_id=array();
           $goods_id= M('goods')->where(array('cat_id'=>859))->select();
           foreach ($goods_id as $key => $value) {
              $order_id= M('order_goods')->where(array('goods_id'=>$value['goods_id']))->select();
           ...
           $sql="select * from tp_order where order_id not in (".$data."-1)";
           if($condition['order_sn']!=''){
            $sql.=" and order_sn =".$condition['order_sn'];
           }
           if($condition['consignee']!=''){
     ...
     * TODO 获取总订单信息
     */
    public function getTotalOrderInfo($order_id)
    {
        //  订单总金额查询语句       
     ...
     * TODO 总订单操作记录
     */
    public function totalOrderActionLog($order_id,$action,$note=''){     
        $order = M('total_order')->where(array('order_id'=>$order_id))->find();
        $data['order_id'] = $order_id;
        $data['action_user'] = session('admin_id');
        $data['action_note'] = $note;
        $data['order_status'] = $order['order_status'];
        ...
        $data['log_time'] = time();
        $data['status_desc'] = $action;        
        return M('total_order_action')->add($data);//订单操作记录
    }
    ...
    public function deliveryHandle($data){
        $order = $this->getTotalOrderInfo($data['order_id']);
        $orderGoods = $this->getTotalOrderGoods($data['order_id']);
        ...
        //TODO 确认发货操作
        M('total_order')->where("order_id=".$data['order_id'])->save($updata);//改变订单状态
        $s = $this->totalOrderActionLog($order['order_id'],'delivery',$data['note']);//操作日志

        //TODO 批量更新子订单状态
        $subOrderList = M('order_goods')->where("total_order_id=".$data['order_id'])->select();//子订单

        foreach ($subOrderList as $val) {
              $count = M('order_goods')->where("order_id=".$val['order_id']." and is_send = 0")->count(); 
              $totalCount = M('order_goods')->where("order_id=".$val['order_id'])->count(); 
              
              if ($count == 0) {
                  $updata['shipping_status'] = 1;
                  M('order')->where("order_id=".$val['order_id'])->save($updata);
                  $s = $this->orderActionLog($val['order_id'],'delivery',$data['note']);//操作日志
