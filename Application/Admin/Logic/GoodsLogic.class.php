...
class GoodsLogic extends RelationModel
{
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        ...
        foreach ($spec_arr_sort as $key =>$val)
        {
         ...
         $keySpecGoodsPrice = M('SpecGoodsPrice')->where('goods_id = '.$goods_id)->getField('key,key_name,price,proposed_price,store_count,bar_code,sku');//规格项
       ...
       foreach ($spec_arr2 as $k => $v) 
       {
            ...
            $keySpecGoodsPrice[$item_key][proposed_price] ? false : $keySpecGoodsPrice[$item_key][proposed_price] = 0; // 价格默认为0
            ...
            $str .="<td><input name='item[$item_key][proposed_price]' value='{$keySpecGoodsPrice[$item_key][proposed_price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
