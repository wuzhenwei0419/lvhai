...
class IndexController extends WechatBaseController {
    ...
    public function index(){                
       ...
       // $ss=$_POST['ss'];
        ...
        // $this->display();
        // echo json_encode($hot_goods);
        // echo json_encode($favourite_goods);
        // echo json_encode($thems);
        $ad=M('ad')->where(array('pid'=>2,'enabled'=>1))->select();
        $data=array('article'=>$article,'favourite_goods'=>$favourite_goods,'thems'=>$thems,'hot_goods'=>$hot_goods,'ad'=>$ad);
