...
//            echo $nickname;die();
   ...
   public function pass(){
    $phone_num = M('users')->where(array('user_id'=>$this->user_id))->getField('mobile');
    $pass = M('application')->where(array('phone_num'=>$phone_num))->getField('pass');
    $p = M('admin')->where(array('user_id'=>$this->user_id))->getField('password');
    //$password = decrypt($p);
    if($phone_num==''){
          $this->success('请先绑定手机',U('Home/User/mobile_validate'));
          // $return_arr = array(
          //                'status' => 1,
          //               'msg'   => '请先绑定手机',                          
          //               'data'  => array('url'=>U('Home/User/mobile_validate')),
          //           );
   ...
   public function del_pass(){
    $phone_num=M('users')->where(array('user_id'=>$this->user_id))->getField('mobile');
    $pass=M('application')->where(array('phone_num'=>$phone_num))->delete();
    //$this->success('重新申请',U('home/user/pass'));
   }
    public function add_pass(){
        $model=M('application');
        $data=I('post.');
        $verify_code = $data['verify_code'];
        //var_dump(print_r($verify_code));die;
        $verify = new Verify();
        if (!$verify->check($verify_code,'add_pass'))
        {
             $res = array('status'=>-1);
             exit(json_encode($res));
        }
        $data['add_time']=time();
        $data['pass']=1; 
        $model->where(array('phone_num'=>$data['referee_phone_num']))->setInc('referee_num',1);
        $list=$model->add($data);
     
         $return_arr = array(
                         'status' => 1,
                        'msg'   => $data,
                        //'data'  => array('url'=>U('Home/User/index')),
                    );
                    $this->ajaxReturn(json_encode($return_arr));
    ...
    {
        //验证码类型
        $type = I('get.type') ? I('get.type') : 'add_pass';
        $config = array(
            'fontSize' => 40,
            'length' => 4,
            'useCurve' => true,
    ...
    public function supplier(){
        $is_check=M('suppliers')->where('users_id ='.$this->user_id)->getField('is_check');
    ...
    public function add_supplier(){
        ...
        $verify = new Verify();
        if (!$verify->check($verify_code,'add_supplier'))
        {
             $res = array('status'=>-1);
             exit(json_encode($res));
        }
        $list=$model->add($data);
        if($list){
             $this->success('上传成功',U('Home/User/supplier'));
        
        }else{
             $this->error('上传失败',U('Home/User/supplier'));   
                }
    ...
    M('suppliers')->where('users_id = '.$this->user_id)->delete();
    $this->assign('操作成功',U('Home/User/supplier'));
   }

    public function supplier_verify()
    {
        //验证码类型
        $type = I('get.type') ? I('get.type') : 'add_supplier';
        $config = array(
            'fontSize' => 40,
            'length' => 4,
            'useCurve' => true,
