...
class GoodsController extends BaseController {
    ...
    */ 
    public function goodsInfo(){
        ...
        $spec_goods_price  = M('spec_goods_price')->where("goods_id = $goods_id")->getField("key,price,proposed_price,store_count"); // 规格 对应 价格 库存表
        M('Goods')->where("goods_id=$goods_id")->save(array('click_count'=>$goods['click_count']+1 )); //统计点击数
        $commentStatistics = $goodsLogic->commentStatistics($goods_id);// 获取某个商品的评论统计
        $point_rate = tpCache('shopping.point_rate');
        $discount=M('discount')->where(array('is_show'=>1))->select();
        $this->assign('discount',$discount);//满减折扣
    ...
    public function supplier_card(){
        $count=M('suppliers')->where('is_check = 2')->count();
        $page = new Page($count, 15);
        $supplier_list = M('suppliers')->where('is_check = 2')->limit($page->firstRow . ',' . $page->listRows)->select();
