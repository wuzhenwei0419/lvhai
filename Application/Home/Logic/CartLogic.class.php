...
class CartLogic extends RelationModel
{
     ...
     */
    function addCart($goods_id,$goods_num,$goods_spec,$act_id=0,$session_id,$user_id = 0)
    {       
        ...
        {            
            $prom = get_goods_promotion($goods_id,$user_id,$act_id);
            $price = $prom['price'];
     ...
     * @param type $user_id 用户id
     */
    function checkGoodNum($goods_id,$goods_num,$goods_spec,$act_id=0,$session_id,$user_id = 0)
    {
    ...
    {                   
        ...
        foreach ($cartList as $k=>$val){
           ...
           $num = M()->query("select nummin,nummax from tp_discount where is_show=1");
             ...
             //dump(print_r($num));die;
            foreach ($num as $key => $value) {
                //dump(print_r($value));die;
...
//        }
        ...
        if($user['oauth']== 'weixin')
        {
            ...
            $wx_content = "你刚刚下了一笔订单:{$order['order_sn']} 已成功,我们将竭诚为您服务!";
